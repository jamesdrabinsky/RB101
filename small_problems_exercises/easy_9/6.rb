...
P:
  ...
  Explicit requirements:
    - Given a string
    - Return an array that contains every word in the string
    to which you have appended a space and the word length.

A:
  - Initialize a local variable `res` to an empty array
  - Convert the string to an array
  - Iterate over the array of string
    - On each iteration concatenate the current string value with a space 
    and an integer (string length)
=end
